...
// Guaranteed link for every notification type — fallback for legacy rows with null link
...
export function NotificationBell() {
  ...
  // KEY FIX: watch for pathname changes — when Next.js navigation succeeds
  // (even if triggered from inside the portal), close the sheet and restore scroll.
  useEffect(() => {
  ...
  // Shared notification row content
  function renderRows(closeSheet?: () => void) {
    ...
    return (
      <ul className="divide-y divide-neutral-50">
        {notifications.map((n) => {
          ...
          const inner = (
            <>
              <div style={{ width: 32, height: 32, borderRadius: "50%", flexShrink: 0, marginTop: 1, display: "flex", alignItems: "center", justifyContent: "center", background: isUnread ? "rgba(234,88,12,0.08)" : "#f9fafb" }}>
                {TYPE_ICON[n.type]?.(n.read)}
              </div>
          ...
          // Desktop: standard Link + close on click
          return (
            <li key={n.id}>
              <Link href={href} onClick={() => setIsOpen(false)} style={rowStyle} className="hover:bg-orange-50 active:bg-orange-100">
                {inner}
              </Link>
            </li>
          );
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "14px 16px 12px", borderBottom: "1px solid #f3f4f6", flexShrink: 0 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          {unreadCount > 0
            ...
            : notifications.length > 0 ? <span style={{ fontSize: 11, color: "#9ca3af" }}>All caught up ✓</span> : null
          }
  ...
  return (
    <>
      <style>{`
        @keyframes notif-in { from{opacity:0;transform:scale(0.95) translateY(-6px)} to{opacity:1;transform:scale(1) translateY(0)} }
      ...
      <div ref={containerRef} className="relative">
        ...
        {!isMobile && isOpen && (
          <div className="notif-dropdown absolute right-0 top-full z-50 mt-2 w-80 overflow-hidden rounded-2xl border border-neutral-200 bg-white"
            ...
            <div className="max-h-80 overflow-y-auto">{renderRows()}</div>
          </div>
        )}
      </div>
      ...
      {isMobile && isOpen && mounted && createPortal(
        <>
          ...
          <div className="notif-sheet"
            style={{ position: "fixed", bottom: 0, left: 0, right: 0, zIndex: 9991, background: "white", borderRadius: "20px 20px 0 0", maxHeight: "75vh", display: "flex", flexDirection: "column", boxShadow: "0 -8px 40px rgba(0,0,0,0.15)" }}>
            ...
            <div style={{ overflowY: "auto", flex: 1, paddingBottom: "env(safe-area-inset-bottom)" }}>
              {/* Pass closeSheet so rows use router.push instead of Link */}
              {renderRows(() => setIsOpen(false))}
            </div>
          </div>
        </>,
        document.body
